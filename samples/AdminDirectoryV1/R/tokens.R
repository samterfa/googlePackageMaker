	...
	tokens.delete <- function(clientId, userKey, gargle_token = gargle::token_fetch(scopes = .endpoints[['directory.tokens.delete']]$scopes), return_response = F){
		...
		req <- gargle::request_develop(endpoint = .endpoints[['directory.tokens.delete']], params = params)
	...
	tokens.get <- function(clientId, userKey, gargle_token = gargle::token_fetch(scopes = .endpoints[['directory.tokens.get']]$scopes), return_response = F){
		params <- as.list(environment())[!names(as.list(environment())) %in% c('return_response', 'gargle_token')]
		req <- gargle::request_develop(endpoint = .endpoints[['directory.tokens.get']], params = params)
		req <- gargle::request_build(method = req$method, path = req$path, params = req$params, body = req$body, token = httr::config(token = gargle_token))
		res <- gargle::request_make(req, encode = 'json')
		if(return_response) return(res)
		httr::content(res)
	...
	tokens.list <- function(userKey, gargle_token = gargle::token_fetch(scopes = .endpoints[['directory.tokens.list']]$scopes), return_response = F){
		params <- as.list(environment())[!names(as.list(environment())) %in% c('return_response', 'gargle_token')]
		req <- gargle::request_develop(endpoint = .endpoints[['directory.tokens.list']], params = params)
		req <- gargle::request_build(method = req$method, path = req$path, params = req$params, body = req$body, token = httr::config(token = gargle_token))
		res <- gargle::request_make(req, encode = 'json')
		if(return_response) return(res)
		httr::content(res)
