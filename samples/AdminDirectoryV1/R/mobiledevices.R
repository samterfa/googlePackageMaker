	...
	mobiledevices.action <- function(customerId, resourceId, action = NULL, gargle_token = gargle::token_fetch(scopes = .endpoints[['directory.mobiledevices.action']]$scopes), return_response = F){
		...
		req <- gargle::request_develop(endpoint = .endpoints[['directory.mobiledevices.action']], params = params)
	...
	#' @param return_response Whether to return the response or the response content. Defaults to FALSE (return response content).
	#' @export
	mobiledevices.delete <- function(customerId, resourceId, gargle_token = gargle::token_fetch(scopes = .endpoints[['directory.mobiledevices.delete']]$scopes), return_response = F){
		params <- as.list(environment())[!names(as.list(environment())) %in% c('return_response', 'gargle_token')]
		req <- gargle::request_develop(endpoint = .endpoints[['directory.mobiledevices.delete']], params = params)
		req <- gargle::request_build(method = req$method, path = req$path, params = req$params, body = req$body, token = httr::config(token = gargle_token))
		res <- gargle::request_make(req, encode = 'json')
		if(return_response) return(res)
		httr::content(res)
	...
	mobiledevices.get <- function(resourceId, customerId, projection = NULL, gargle_token = gargle::token_fetch(scopes = .endpoints[['directory.mobiledevices.get']]$scopes), return_response = F){
		params <- as.list(environment())[!names(as.list(environment())) %in% c('return_response', 'gargle_token')]
		req <- gargle::request_develop(endpoint = .endpoints[['directory.mobiledevices.get']], params = params)
		req <- gargle::request_build(method = req$method, path = req$path, params = req$params, body = req$body, token = httr::config(token = gargle_token))
		res <- gargle::request_make(req, encode = 'json')
		if(return_response) return(res)
		httr::content(res)
	...
	#' @param return_response Whether to return the response or the response content. Defaults to FALSE (return response content).
	#' @export
	mobiledevices.list <- function(customerId, orderBy = NULL, sortOrder = NULL, projection = NULL, maxResults = NULL, pageToken = NULL, query = NULL, gargle_token = gargle::token_fetch(scopes = .endpoints[['directory.mobiledevices.list']]$scopes), return_response = F){
		params <- as.list(environment())[!names(as.list(environment())) %in% c('return_response', 'gargle_token')]
		req <- gargle::request_develop(endpoint = .endpoints[['directory.mobiledevices.list']], params = params)
		req <- gargle::request_build(method = req$method, path = req$path, params = req$params, body = req$body, token = httr::config(token = gargle_token))
		res <- gargle::request_make(req, encode = 'json')
		if(return_response) return(res)
		httr::content(res)
