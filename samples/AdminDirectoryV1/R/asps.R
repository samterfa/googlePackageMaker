	...
	asps.delete <- function(userKey, codeId, gargle_token = gargle::token_fetch(scopes = .endpoints[['directory.asps.delete']]$scopes), return_response = F){
		...
		req <- gargle::request_develop(endpoint = .endpoints[['directory.asps.delete']], params = params)
	...
	asps.get <- function(userKey, codeId, gargle_token = gargle::token_fetch(scopes = .endpoints[['directory.asps.get']]$scopes), return_response = F){
		params <- as.list(environment())[!names(as.list(environment())) %in% c('return_response', 'gargle_token')]
		req <- gargle::request_develop(endpoint = .endpoints[['directory.asps.get']], params = params)
		req <- gargle::request_build(method = req$method, path = req$path, params = req$params, body = req$body, token = httr::config(token = gargle_token))
		res <- gargle::request_make(req, encode = 'json')
		if(return_response) return(res)
		httr::content(res)
	...
	asps.list <- function(userKey, gargle_token = gargle::token_fetch(scopes = .endpoints[['directory.asps.list']]$scopes), return_response = F){
		params <- as.list(environment())[!names(as.list(environment())) %in% c('return_response', 'gargle_token')]
		req <- gargle::request_develop(endpoint = .endpoints[['directory.asps.list']], params = params)
		req <- gargle::request_build(method = req$method, path = req$path, params = req$params, body = req$body, token = httr::config(token = gargle_token))
		res <- gargle::request_make(req, encode = 'json')
		if(return_response) return(res)
		httr::content(res)
