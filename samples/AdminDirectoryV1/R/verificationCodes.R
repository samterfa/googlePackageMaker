	...
	#' @export
	verificationCodes.generate <- function(userKey, gargle_token = gargle::token_fetch(scopes = .endpoints[['directory.verificationCodes.generate']]$scopes), return_response = F){
		params <- as.list(environment())[!names(as.list(environment())) %in% c('return_response', 'gargle_token')]
		req <- gargle::request_develop(endpoint = .endpoints[['directory.verificationCodes.generate']], params = params)
	...
	#' @param return_response Whether to return the response or the response content. Defaults to FALSE (return response content).
	#' @export
	verificationCodes.invalidate <- function(userKey, gargle_token = gargle::token_fetch(scopes = .endpoints[['directory.verificationCodes.invalidate']]$scopes), return_response = F){
		params <- as.list(environment())[!names(as.list(environment())) %in% c('return_response', 'gargle_token')]
		req <- gargle::request_develop(endpoint = .endpoints[['directory.verificationCodes.invalidate']], params = params)
		req <- gargle::request_build(method = req$method, path = req$path, params = req$params, body = req$body, token = httr::config(token = gargle_token))
		res <- gargle::request_make(req, encode = 'json')
		if(return_response) return(res)
		httr::content(res)
	...
	#' @param return_response Whether to return the response or the response content. Defaults to FALSE (return response content).
	#' @export
	verificationCodes.list <- function(userKey, gargle_token = gargle::token_fetch(scopes = .endpoints[['directory.verificationCodes.list']]$scopes), return_response = F){
		params <- as.list(environment())[!names(as.list(environment())) %in% c('return_response', 'gargle_token')]
		req <- gargle::request_develop(endpoint = .endpoints[['directory.verificationCodes.list']], params = params)
		req <- gargle::request_build(method = req$method, path = req$path, params = req$params, body = req$body, token = httr::config(token = gargle_token))
		res <- gargle::request_make(req, encode = 'json')
		if(return_response) return(res)
		httr::content(res)
